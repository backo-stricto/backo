...
class DBSqlite3Connector(DBHandler):
    ...
    def connect(self):
        ...
        try:
            if not self._db:
                self._db = sqlite3.connect(self._db_file, **self._db_kwargs)
    ...
    def _drop_table(self, table_name: str, key_path: list[str], model: dict) -> None:
        ...
        log.debug(f"DELETE FROM {table_name}")
    ...
    def _reccursive_check_structure(
        ...
        transformer = self.get_transformer_by_key(
            key_path
        )  # , table_name, model["types"])
        store_in_db = True
        db_path = key_path
        db_model = model
        if transformer:
            ...
            db_model = transformer.get_db_model(model, key_path)
        if store_in_db:
            f = SqlFieldDescription()
            f.create("_".join(db_path), db_model)
            tp.add_backo_field(f)
    ...
    def _search_in_table(
        ...
        log.debug(f"SELECT * FROM {table_name} WHERE {id_name} == {sqlite3_id}")
    ...
    def load(
        ...
        # If the key path is None, use the "_" field.  (used during List(String()) by example)
        db_key = "_".join(key_path) if key_path else "_"
    ...
    def _create_in_table(
        ...
        :rtype: int
        """
        log.debug(f"Create in table {table_name} {keys_values}")
        r = tuple(map(tuple, zip(*keys_values)))
        t = ("?",) * len(r[0])
        log.debug(
            f"INSERT INTO {table_name} {r[0]} VALUES ({', '.join(t) }) with {r[1]}"
        )
    ...
    def _reccursive_create(
        ...
        :param keys_values: list of key - values
        :type keys_values: list[ tuple[ str, Any ]]
        :param _id_per_table: the _id created per table
        :type _id_per_table: dict
        ...
        if "sub_scheme" in model and isinstance(o, dict):
            ...
            for k, v in sorted(
                ...
                new_key_path.append(k)
                if k in o:
                    self._reccursive_create(
                        table_name, new_key_path, o[k], v, keys_values, _id_per_table
                    )
            return

        # A List
        if "sub_type" in model and isinstance(o, list):
            if keys_values and not table_name in _id_per_table:
                _id_per_table[table_name] = self._create_in_table(
                    table_name, keys_values
                )
                keys_values.clear()

            if not table_name in _id_per_table:
                raise DBError("gargle")
            sub_table_name = f"{table_name}_{'_'.join(key_path)}"
            for d in o:
                kv = [(f"{table_name}_id", _id_per_table[table_name])]
                self._reccursive_create(
                    sub_table_name, [], d, model["sub_type"], kv, _id_per_table
                )
    ...
    def create(self, o: Any) -> str:  # pylint: disable=unused-argument
        ...
        _id_per_table = {}
        self._reccursive_create(
            self._table_name, [], o, self.model, keys_values, _id_per_table
        )
        if keys_values:
            _id_per_table[self._table_name] = self._create_in_table(
                self._table_name, keys_values
            )
        self._db.commit()

        return str(_id_per_table[self._table_name])
    ...
    def _insert_in_table(self, table_name: str, _id: int, keys_values: dict) -> None:
        ...
        v.append(_id)
        log.debug(
            f'UPDATE {table_name} SET {", ".join(l)} WHERE id == ? with {tuple(v)}'
        )
        self._cursor.execute(
    ...
    def _reccursive_save(
        ...
        if "sub_type" in model and isinstance(o, list):
            ...
            for d in o:
                ...
                self._reccursive_create(
                    sub_table_name, [], d, model["sub_type"], kv, {}
                )
                self._create_in_table(sub_table_name, kv)
                kv.clear()
